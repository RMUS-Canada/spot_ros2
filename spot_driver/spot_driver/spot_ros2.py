...
class SpotROS(Node):
    ...
    def handle_remove_all_policies(self, request: Trigger.Request, response: Trigger.Response) -> Trigger.Response:
        """ROS service handler to remove all keepalive policies from the robot"""
        if self.spot_wrapper is None:
            response.success = False
            response.message = "Spot wrapper is undefined"
            return response
